class Deployment < ActiveRecord::Base
  ...
  def deploy_in_background! 
    unless RAILS_ENV == 'test'   
      ...
      cmd = "sh -c \"cd #{RAILS_ROOT} && ruby script/runner -e #{RAILS_ENV} ' deployment = Deployment.find(#{self.id}); deployment.prompt_config = #{self.prompt_config.inspect.gsub('"', '\"')} ; Webistrano::Deployer.new(deployment).invoke_task! ' >> #{RAILS_ROOT}/log/#{RAILS_ENV}.log 2>&1\" &"
	puts "---- ABOUT TO EXECUTE THIS COMMAND:\n\n#{cmd}"
	system(cmd)
    end
  end
